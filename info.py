...
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '597083078').split()]
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1002855139376').split()] #Channel id for auto indexing ( make sure bot is admin )
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '1002554393480').split()]
...
# MongoDB information
DATABASE_URI = environ.get('DATABASE_URI', "mongodb+srv://pulikkalhouse2713:ipXHSk6Ye6WgHg8Y@cluster0.hjeg0zj.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0")
...
PREMIUM_LOGS = int(environ.get('PREMIUM_LOGS', '1002838937528'))
